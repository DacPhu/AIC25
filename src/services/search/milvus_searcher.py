...
class Searcher(BaseSearch):
    ...
    def get(self, param_id):
        return self._database.get(param_id)
    ...
    @staticmethod
    def _process_advance(advance_query, result, ocr_weight, ocr_threshold):
    ...
    def _simple_search(self, processed, filter_p, offset, limit, nprobe, model):
        text_features = (
            self._models[model].get_text_features(processed["queries"]).tolist()
        )
        filter_p = self._combine_videos_filter(filter_p, processed["video_ids"])
    ...
    @staticmethod
    def _combine_videos_filter(filter_p, video_ids):
        video_ids_filter = " || ".join(
            [f'frame_id like "{x.strip()}#%"' for x in video_ids]
        )
        filter_empty = len(filter_p) == 0
        if len(video_ids) > 0:
            video_ids_filter = "(" + video_ids_filter + ")"
            if not filter_empty:
                video_ids_filter = f"{filter} && {video_ids_filter}"
        return video_ids_filter
    ...
    def _get_videos(self, video_ids, offset, limit, selected):
        ...
        if query_hash in self.cache:
            ...
            videos = self._database.query(video_ids_filter, 0, 10000)
    ...
    def search(
        ...
        if no_query and no_advance:
            ...
            return self._simple_search(
                processed, filter_p, offset, limit, nprobe, model
            )
        else:
    ...
    def search_similar(
        ...
        record = self._database.get(p_id)
        if len(record) == 0:
